...
extern bool AreTypesCompatible(Common *node, Symbol *T1, Symbol *T2);
...
ConstantValue *NegExpr::EvaluateConst(void)
{
	...
	if (cVal) {
		if (type == INTEGER) cVal->intVal = -cVal->intVal;
		else if (type == REAL) cVal->realVal = -cVal->realVal;
	}
...
ConstantValue *SubExpr::EvaluateConst(void)
{
	...
	} else if (AreTypesCompatible(this, left->Get_type(), INTEGER) 
			&& AreTypesCompatible(this, right->Get_type(), REAL)) {
		 cVal->realVal = (double)l->intVal - r->realVal;
	} else if (AreTypesCompatible(this, left->Get_type(), REAL) 
			&& AreTypesCompatible(this, right->Get_type(), INTEGER)) {
...
ConstantValue *RealDivExpr::EvaluateConst(void)
{
	...
	//    ----------------------------------------------------------------------------------
	if (AreTypesCompatible(this, right->Get_type(), INTEGER) && r->intVal == 0) {
		SemantError(this, "Division by 0 in constant evaluation");
		cVal->realVal = 0.0;
		goto exit;
	} else if (AreTypesCompatible(this, right->Get_type(), REAL) && r->realVal == 0.0) {
		SemantError(this, "Division by 0 in constant evaluation");
		cVal->realVal = 0.0;
		goto exit;
	}
...
ConstantValue *GTExpr::EvaluateConst(void)
{
	...
	} else if (AreTypesCompatible(this, left->Get_type(), STRING) 
			&& AreTypesCompatible(this, right->Get_type(), STRING)) {
